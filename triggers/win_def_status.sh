...
echo "[OK] Item добавлен: ID=$ITEM_ID"

TRIGGER_RESP=$(curl -s -X POST -H 'Content-Type: application/json' \
-d '{
  "jsonrpc": "2.0",
  "method": "trigger.create",
  "params": {
    "description": "Windows Defender components are DISABLED on {HOST.NAME}",
    "expression": "{'$TEMPLATE_HOST':windows.defender.status.strlen()} > 1 and {'$TEMPLATE_HOST':windows.defender.status.last()} <> \"0\"",
    "priority": 4
  },
  "auth": "'$AUTH_TOKEN'",
  "id": 4
...
TRIGGER_ID=$(echo "$TRIGGER_RESP" | jq -r '.result.triggerids[0]')

if [[ -z "$TRIGGER_ID" || "$TRIGGER_ID" == "null" ]]; then
  echo "[ERROR] Триггер не создан:"
  echo "$TRIGGER_RESP"
  exit 1
fi
echo "[SUCCESS] Trigger создан: ID=$TRIGGER_ID"
